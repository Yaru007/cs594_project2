...
local function createModel(opt)
   ...
   local nconv = opt.nconv
   
   -- The new Residual Unit in [a]
   local function bottleneck(nInputPlane, nOutputPlane, stride, nconv)
   ...
   local function layer(block, nInputPlane, nOutputPlane, count, stride,nconv)
      local s = nn.Sequential()

      s:add(block(nInputPlane, nOutputPlane, stride,nconv))
      for i=2,count do
   ...
   do
      ...
      model:add(layer(bottleneck, nStages[1], nStages[2], n, 1,nconv)) -- Stage 1 (spatial size: 32x32)
      model:add(layer(bottleneck, nStages[2], nStages[3], n, 2,nconv)) -- Stage 2 (spatial size: 16x16)
      model:add(layer(bottleneck, nStages[3], nStages[4], n, 2,nconv)) -- Stage 3 (spatial size: 8x8)
